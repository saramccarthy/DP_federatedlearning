...
def distorted_inputs(client):
  ...
  images, labels = cifar10_input.distorted_inputs(data_dir=data_dir, client=client,
...
def buildModel():
  ...
  #
  # conv1
  images = tf.placeholder(tf.float32, [FLAGS.batch_size, 24, 24, 3])
  ...
  logits = tf.placeholder(tf.float32, softmax_linear.shape)
