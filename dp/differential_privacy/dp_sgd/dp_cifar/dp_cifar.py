...
def Train(cifar_train_file, mnist_test_file, network_parameters, num_steps,
  ...
  with tf.Graph().as_default(), tf.Session() as sess, tf.device('/cpu:0'):
    ...
    # Load pre-trained CIFAR-100 layers
    use_pretrained_cifar100 = True
    if use_pretrained_cifar100:
      cif100_dir = "models/cifar100/"
      all_vars = tf.global_variables()
      restore_vars = [v for v in all_vars if not v.name.startswith('logit')]
      saver = tf.train.Saver(restore_vars)
      saver.restore(sess, tf.train.latest_checkpoint(cif100_dir))
